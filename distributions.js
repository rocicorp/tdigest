...
function distributions(nruns, nsamples, npercentiles) {
  [uniform, gaussian, chisq, exponential, make_brownian()].map(function (dist) {
    var fname = dist.toString().match(/function ([^\(]+)/)[1];
    console.log(
      fname + ': ' + nruns + ' runs digesting ' + nsamples + ' points:',
    );
    var errs = [0, 0];
...
function compare(d1, d2, N) {
  ...
  var rmse = 0;
  var scale = Math.max(
    Math.abs(d1.percentile(0)),
    Math.abs(d1.percentile(1)),
    Math.abs(d2.percentile(0)),
    Math.abs(d2.percentile(1)),
  );
  for (var i = 0; i <= N; i += 1) {
